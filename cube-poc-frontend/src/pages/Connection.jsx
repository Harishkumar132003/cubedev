...
const DATABASES = [
  {
    ...
    defaultPort: '5432',
    icon: (
      <svg viewBox="-4 0 264 264" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" fill="#000000">
        <g strokeWidth="0" />
...
function Connection() {
  ...
  if (result) {
    return (
      <div className="card success-card">
        ...
        <p className="muted">
          Connected to <strong>{selectedDb.name}</strong> at{' '}
          <code>
            {lastValues.host}:{lastValues.port}
          </code>
        </p>
        {result.message && <p className="muted">{result.message}</p>}
        {result.cubesGenerated?.length > 0 && (
          <details className="config-details">
            <summary>{result.cubesGenerated.length} cube(s) generated</summary>
            <ul className="config-list">
              {result.cubesGenerated.map((name) => (
                <li key={name}>{name}</li>
              ))}
            </ul>
          </details>
  ...
  if (error) {
    return (
      <div className="card error-card">
        ...
        {progress.length > 0 && (
          <details className="config-details">
            <summary>Steps before failure</summary>
            <ul className="config-list">
              {progress.map((msg, i) => (
                <li key={i}>{msg}</li>
              ))}
  ...
  return (
    <div className="card">
      {!selectedDb ? (
        <DatabasePicker databases={DATABASES} onSelect={setSelectedDb} />
      ) : (
        <>
          <ConnectionForm
            db={selectedDb}
            onChange={() => !submitting && setSelectedDb(null)}
          ...
          {submitting && (
            <div className="config-progress">
              <div className="config-progress-title">
                ...
                <span>Configuring connection</span>
              </div>
              <ul className="config-list">
                {progress.map((msg, i) => (
                  <li key={i}>{msg}</li>
                ...
                {progress.length === 0 && (
                  <li className="muted">Starting…</li>
                )}
              </ul>
            </div>
          )}
